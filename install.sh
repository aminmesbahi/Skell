...
detect_os() {
  ...
  case "$OS" in
    ...
    linux)  OS="linux"  ;;
    mingw*|msys*|cygwin*)
      error "Use PowerShell on Windows: irm https://raw.githubusercontent.com/aminmesbahi/skell/main/install.ps1 | iex"
      ;;
...
install() {
  ...
  info "Downloading skell ${VERSION} for ${OS}/${ARCH}..."
...
verify() {
  if command -v "$BINARY_NAME" >/dev/null 2>&1; then
    info ""
    info "Successfully installed skell to $INSTALL_DIR/$BINARY_NAME"
    info ""
    "$BINARY_NAME" --version
    info ""
    info "Get started:"
